...
import utils
from dataloaders.data_rgb import get_training_data, get_validation_data,get_training_data_fusion,get_validation_data_fusion
...
train_dataset = get_training_data_fusion('train', img_options_train)
...
val_dataset = get_validation_data_fusion('test')
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):    
        ...
        restored = model_restoration(input_,input_2)
        restored = torch.clamp(restored,0,1)  
        
        #loss = criterion(restored, target)
        loss = torch.mean(torch.abs(restored-target))
        ...
        with torch.no_grad():
            ...
            for ii, data_val in enumerate(tqdm(val_loader), 0):
                ...
                if save_images:
                    ...
                    input_2 = input_2.permute(0, 2, 3, 1).cpu().detach().numpy()
                    restored = restored.permute(0, 2, 3, 1).cpu().detach().numpy()

                    for batch in range(input_.shape[0]):
                        temp = np.concatenate((input_[batch]*255,input_2[batch]*255, restored[batch]*255, target[batch]*255),axis=1)
                        os.makedirs(os.path.join(result_dir, str(epoch), str(i)),exist_ok=True)

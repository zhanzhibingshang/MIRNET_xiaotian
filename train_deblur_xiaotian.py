...
opt = Config('training_xiaotian.yml')
...
from networks.MIRNet_model import MIRNet, MIRNet_xiaotian
...
######### Model ###########
model_restoration = MIRNet(in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=4, n_MSRB=1, height=2, width=1, bias=False)
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):    
        ...
        if  Loss_mode == 'ssim':
            loss = 1 - ssim_loss(restored,target)
        else:
